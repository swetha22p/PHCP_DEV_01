...
const onLocationChange = (value) => {
    switch (value) {
      case 'hyderabad':
        form.setFieldsValue({
          note: 'You selected Hyderabad!',
        });
        break;
      case 'vizag':
        form.setFieldsValue({
          note: 'You selected Vizag!',
        });
        break;
      case 'vijayawada':
        form.setFieldsValue({
          note: 'You selected Vijayawada!',
        });
        break;
      case 'bhimavaram':
        form.setFieldsValue({
          note: 'You selected Bhimavaram!',
        });
        break;
      default:
        form.setFieldsValue({
          note: 'No location selected!',
        });
    }
  };
...
const normFile = (e) => {
    ...
    const fetchAllDrives = async () => {
      try {
          const response = await axios.get('http://127.0.0.1:5401/get_drives');
          console.log(response)
          const driveNames = response.data.drives.map(drive => drive.name);
          console.log('Drive Names:', driveNames);
          setDrives(driveNames)
          return driveNames; // Optionally, you can return the array of drive names
      } catch (error) {
...
const fetchLatestDocuments = async () => {
  try {
    const response = await axios.get('https://pl-api.iiit.ac.in/rcts/phcp2/api/get-latest-documents');
        ...
        >
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: translations["Please input your username!"],
              },
            ]}
          >
          ...
          <Form.Item
            label={translations["Input Number"]}
            name="InputNumber"
            rules={[{ required: true, message: translations["Please input!"] }]}
          >
          ...
          >
            <Select
              placeholder={translations["Select a option and change input text above"]}
              onChange={(value) => {
                setGender(value);
          ...
          >
            <Select
              placeholder={translations["Select a location"]}
              onChange={(value) => {
                setLocation(value);
              ...
              allowClear
            >
              <Option value="hyderabad">{translations["Hyderabad"]}</Option>
              <Option value="vizag">{translations["Vizag"]}</Option>
              <Option value="vijayawada">{translations["Vijayawada"]}</Option>
              <Option value="bhimavaram">{translations["Bhimavaram"]}</Option>
            </Select>
          </Form.Item>
        </Form>
      </div>
      <div className="right">
        <Form.Item label={translations["Upload"]} style={{ marginLeft: 110 }} valuePropName="fileList" getValueFromEvent={normFile}>
          {saved ? (
            <>
              <img
                src={imageUrl}
        ...
        <Modal
          title={translations["Capture Image"]}
          visible={visible}
        ...
        <Form.Item >
          {!audioFileName && (
            ...
            >
              <AudioOutlined />
              <div style={{ marginTop: 8 }}>{translations["Record Audio"]}</div>
            </button>
          )}
        </Form.Item>
        <Form.Item>
          {audioFileName && (
            <div>
              <label>{translations["Audio"]}:</label>
              <span
...
>
    <Select>
        {drives.map((drive, index) => (
            <Select.Option key={index} value={drive}>
                {drive}
            </Select.Option>
        ))}
    </Select>
        ...
        <Modal
          title={translations["Record Audio"]}
          visible={modalAudioVisible}
          onCancel={handleCloseAudioModal}
          footer={[
            <Button
              key="toggle"
              onClick={audioPresent ? () => alert(translations["Please reset the audio before starting a new recording."]) : (recording ? stopRecording : recordAudio)}
              style={{ backgroundColor: recording ? 'red' : 'green' }}
            >
              {recording ? translations["Stop Recording"] : translations["Start Recording"]}
            </Button>,
          ...
          {audioBlob && (
            <div>
              <audio controls>
                ...
                {translations["Your browser does not support the audio element."]}
              </audio>
            </div>
          )}
        </Modal>
        ...
        <Modal
          title={translations["Audio Player"]}
    ...
    <Button type="primary" style={{ marginLeft: 110 }} onClick={handleSubmit}>
      {translations["Submit"]}
    </Button>
  </div>
);
